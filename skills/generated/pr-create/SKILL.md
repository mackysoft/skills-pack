...
PR タイトルと本文は `$writing` で文章前提を固定してから書く。
言語は、ユーザー指定、既存 PR、Issue 本文、リポジトリの同種 PR、会話言語の順に判断する。
PR タイトルと本文の言語を揃える。
本文の見出しは `Summary`、`Changelog`、`Verification`、`Risks / Rollback` に固定し、本文の言語に合わせて翻訳しない。

...
本文の見出しは `Summary`、`Scope`、`References`、`Changelog`、`Verification`、`Risks / Rollback` の順に置き、本文の言語に合わせて翻訳しない。
`Closes #<N>` のような GitHub の構文は原文のまま置く。

本文は `Summary`、`Scope`、`Verification` を基本にする。
レビュー時に確認すべき資料、仕様、外部URL、リポジトリ内の相対パスがある場合は `References` を足す。
`References` にはローカル環境の絶対パスを書かない。

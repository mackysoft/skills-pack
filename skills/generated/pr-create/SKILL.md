...
## 目的
現在作業から PR を作成または更新する。

PR の base を決め、必要なら `$sync-latest` で最新化し、検証、`$push`、本文作成、PR 作成または更新まで進める。
...
任意セクションは、必要な場合だけ次の表の位置に置く。
本文の見出しは本文の言語に合わせて翻訳しない。
`Closes #<N>` のような GitHub の構文は原文のまま置く。

| 見出し | 必須 | 位置 | 内容 |
| --- | --- | --- | --- |
| `Summary` | 必須 | 先頭 | 変更の要点。レビュー前に把握すべき結論を短く書く。 |
| `Changelog` | 任意 | `Summary` の後 | 利用判断に使う変更説明が必要な場合に置く。項目は `$changelog` で作り、見出しはこのスキルで置く。 |
| `Scope` | 必須 | `Summary` または `Changelog` の後 | 変更した範囲、生成物、影響する機能や文書を列挙する。 |
| `Verification` | 必須 | `Scope` の後 | 実行した検証、確認できた結果、実行できなかった検証を書く。 |
| `References` | 任意 | `Verification` の後 | レビュー時に確認すべき資料、仕様、外部URL、リポジトリ内の相対パスを書く。ローカル環境の絶対パスは書かない。 |
| `Risks / Rollback` | 任意 | 最後 | レビュー判断に必要な具体的なリスク、未確認事項、通常のrevertだけでは足りない戻し方を書く。 |
